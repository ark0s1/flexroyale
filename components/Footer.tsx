  ...
  return (
    <footer className="mt-16 py-10 px-4">
      ...
      <div className="text-center space-y-4">
        <p className="font-gaming text-lg tracking-widest text-gray-600">FLEXROYALE</p>
        ...
        <nav aria-label="Grades FlexRoyale" className="flex flex-wrap items-center justify-center gap-2">
          ...
          {GRADES.map(({ g, slug }) => (
            <Link
              key={slug}
              href={`/grade/${slug}`}
              className="text-xs font-bold text-gray-500 hover:text-bone border border-white/10 hover:border-clay px-2 py-1 transition-colors"
            >
              {g}
            </Link>
          ))}
        ...
        <p className="text-gray-600 text-xs max-w-xl mx-auto leading-relaxed">
          ...
          <a
            ...
            className="text-dustyblue hover:text-bone underline transition-colors"
          >
